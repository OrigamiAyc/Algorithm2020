...
{
	int key;
	union key_expand_type
	{
		int key;
		union key_expand_type other;
	};
} key_t;
...
void rbt_transplant(rbt_t *u, rbt_t *v);

void rbt_delete(rbt_t *z);

void rbt_delete_fixup(rbt_t *z);

/**
 * @brief rotate the subtree, choose center-node's rchild as its new parent
 * @param[in] node x, which has a non-nil rchild y
 * @param[global] the tree, which is rbt_t *root and defined as global
 **/
void rbt_left_rotate(rbt_t *x)
{
